  ...
  render() {
    ...
    function getSumOfTime(eventsArr) {
      return eventsArr.reduce((prevVal, elem) => {
        if (elem.stoppedOn > 0)
          return prevVal + (elem.stoppedOn - elem.startedOn);
        return prevVal + (new Date().getTime() - elem.startedOn);
      }, 0);
    }

    function getOnlinePercentage(registeredOn, leftOn) {
      const totalUserOnlineTime =
        (leftOn > 0 ? leftOn : new Date().getTime()) - registeredOn;
        ...
        <thead>
          <tr className="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b bg-gray-100">
            <th className="px-4 py-3">
              <FormattedMessage
                id="app.learningDashboard.participantsTable.colParticipant"
                defaultMessage="Participant"
              />
            </th>
            <th className="px-4 py-3 text-center">
              <FormattedMessage
                id="app.learningDashboard.participantsTable.colOnline"
                defaultMessage="Online time"
              />
            </th>
            <th className="px-4 py-3 text-center">
              <FormattedMessage
                id="app.learningDashboard.participantsTable.colTalk"
                defaultMessage="Talk time"
              />
            </th>
            <th className="px-4 py-3 text-center">
              <FormattedMessage
                id="app.learningDashboard.participantsTable.colWebcam"
                defaultMessage="Webcam Time"
              />
            </th>
            <th className="px-4 py-3 text-center">
              <FormattedMessage
                id="app.learningDashboard.participantsTable.colMessages"
                defaultMessage="Messages"
              />
            </th>
            <th className="px-4 py-3 text-left">
              <FormattedMessage
                id="app.learningDashboard.participantsTable.colEmojis"
                defaultMessage="Emojis"
              />
            </th>
            <th className="px-4 py-3 text-center">
              <FormattedMessage
                id="app.learningDashboard.participantsTable.colRaiseHands"
                defaultMessage="Raise Hand"
              />
            </th>
            <th className="px-4 py-3">
              <FormattedMessage
                id="app.learningDashboard.participantsTable.colStatus"
        ...
        <tbody className="bg-white divide-y">
          ...
          Object.values(allUsers || {}).length > 0 ? (
            Object.values(allUsers || {})
              .sort((a, b) => {
                if (a.isModerator === true && b.isModerator === false)
                  return -1;
              ...
              .map((user) => (
                <tr key={user} className="text-gray-700">
                  <td className="px-4 py-3">
                    <div className="flex items-center text-sm">
                      <div className="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                        ...
                        {/*     alt="" loading="lazy" /> */}
                        <div
                          className={`border-2 border-gray-800 items-center ${
                            user.isModerator ? "rounded-md" : "rounded-full"
                          }`}
                        >
                          <svg
                  ...
                  <td className="px-4 py-3 text-sm text-left">
                    {user.emojis.filter((emoji) => emoji.name === "away")
                      .length > 0 ? (
                    ...
                    {user.emojis.filter((emoji) => emoji.name === "neutral")
                      .length > 0 ? (
                      <div className="text-xs">
                    ...
                    {user.emojis.filter((emoji) => emoji.name === "confused")
                      .length > 0 ? (
                      <div className="text-xs">
                        <i className="icon-bbb-undecided text-sm" />
                        &nbsp;
                    ...
                    {user.emojis.filter((emoji) => emoji.name === "sad")
                      .length > 0 ? (
                      <div className="text-xs">
                    ...
                    {user.emojis.filter((emoji) => emoji.name === "happy")
                      .length > 0 ? (
                      <div className="text-xs">
                    ...
                    {user.emojis.filter((emoji) => emoji.name === "applause")
                      .length > 0 ? (
                      <div className="text-xs">
                    ...
                    {user.emojis.filter((emoji) => emoji.name === "thumbsUp")
                      .length > 0 ? (
                      <div className="text-xs">
                    ...
                    {user.emojis.filter((emoji) => emoji.name === "thumbsDown")
                      .length > 0 ? (
                      <div className="text-xs">
                  ...
                  <td className="px-4 py-3 text-sm text-center">
                    {user.emojis.filter((emoji) => emoji.name === "raiseHand")
                      .length > 0 ? (
                      <span>
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                  ...
                  <td className="px-4 py-3 text-xs">
                    {user.leftOn > 0 ? (
                      <span className="px-2 py-1 font-semibold leading-tight text-red-700 bg-red-100 rounded-full">
                        <FormattedMessage
                          id="app.learningDashboard.participantsTable.userStatusOffline"
                      ...
                      <span className="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full">
                        <FormattedMessage
                          id="app.learningDashboard.participantsTable.userStatusOnline"
            ...
            <tr className="text-gray-700">
              <td colSpan="8" className="px-4 py-3 text-sm text-center">
                <FormattedMessage
                  id="app.learningDashboard.participantsTable.noUsers"
